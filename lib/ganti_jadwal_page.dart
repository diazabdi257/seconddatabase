...
class _GantiJadwalPageState extends State<GantiJadwalPage> {
  ...
  void initState() {
    ...
    _sportNode =
        _currentBooking.midtransOrderId.toUpperCase().startsWith('BADMINTON')
            ? 'bookings_badminton'
            : 'bookings_futsal';
  ...
  void _initializeDateRangeAndSelection() {
    DateTime originalBookingDate = DateFormat(
      'yyyy-MM-dd',
    ).parse(_currentBooking.bookingDate);
    ...
    _minSelectableDate =
        todayDateOnly.isAfter(minRange) ? todayDateOnly : minRange;
    ...
    while (currentDate.isBefore(
      _maxSelectableDate.add(const Duration(days: 1)),
    )) {
      // +1 karena isBefore
    ...
    if (originalBookingDate.isAfter(
          _minSelectableDate.subtract(const Duration(days: 1)),
        ) &&
        originalBookingDate.isBefore(
          _maxSelectableDate.add(const Duration(days: 1)),
        )) {
    ...
    } else if (!_availableDatesForReschedule.contains(_selectedNewDate) &&
        _availableDatesForReschedule.isNotEmpty) {
      _selectedNewDate =
          _availableDatesForReschedule
              .first; // Fallback ke tanggal pertama yang tersedia
    }
  }
  ...
  Future<void> _fetchBookedSlotsForNewDate() async {
    ...
    try {
      DataSnapshot snapshot = await bookingPathRef.once().then(
      ...
      if (snapshot.exists && snapshot.value != null) {
        ...
        bookings.forEach((key, value) {
          if (value is Map) {
            // PERBAIKAN: Pengecekan status yang lebih baik untuk menghindari double booking
            String status =
            ...
            // Slot dianggap booked jika sudah dibayar, dan BUKAN booking yang sedang diganti ini
            if (isPaid && key != _currentBooking.key) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Gagal memuat jadwal untuk tanggal baru: $error"),
          ),
        );
      }
    }
  ...
  // Fungsi validasi tetap sama, akan digunakan untuk memberikan pesan error
  bool _isSelectionValid(List<int> selected) {
    if (selected.isEmpty) return false;
    if (selected.length != _originalDurationInHours) return false;
    selected.sort();
  ...
  Future<void> _confirmReschedule() async {
    // PERBAIKAN: Memberikan pesan error yang lebih spesifik
    if (_selectedNewHourIndexes.length != _originalDurationInHours) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "Harap pilih $_originalDurationInHours jam, sesuai durasi booking asli Anda.",
          ),
        ),
      );
      return;
    ...
    if (!_isSelectionValid(_selectedNewHourIndexes)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Pilih slot jam secara berurutan.")),
      );
      return;
    }
    ...
    String oldPath =
        '${_sportNode}/${_currentBooking.fieldId}/${_currentBooking.bookingDate}/${_currentBooking.key}';
    String newPath =
        '${_sportNode}/${_currentBooking.fieldId}/$formattedNewDate/${_currentBooking.key}';

    // Data baru untuk booking
    Map<String, dynamic> newBookingData = {
      'B_id': _currentBooking.midtransOrderId,
      'bookingDate': formattedNewDate,
      'startTime': newStartTime,
      'endTime': newEndTime,
      'fieldId': _currentBooking.fieldId,
      'gross_amount': _currentBooking.grossAmount,
      'key': _currentBooking.key,
      'midtrans_order_id': _currentBooking.midtransOrderId,
      'payment_status':
          _currentBooking.paymentStatus, // Status pembayaran tetap sama
      'userName': _currentBooking.userName,
      'scheduleChangeCount':
          _currentBooking.scheduleChangeCount - 1, // Kurangi kesempatan
      'timestamp': ServerValue.timestamp, // Update timestamp
      'userId':
          FirebaseAuth.instance.currentUser?.uid, // Simpan juga userId jika ada
    };

    try {
      Map<String, dynamic> multiPathUpdate = {
        oldPath: null,
        newPath: newBookingData,
      };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:
          _isLoadingData && _availableDatesForReschedule.isEmpty
              ...
              : Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      "Lapangan: ${widget.initialBooking.fieldDisplayName}",
                    ...
                    Text(
                      "Jadwal Awal: ${widget.initialBooking.formattedDisplayDate}, ${widget.initialBooking.timeRangeDisplay}",
                    ),
                    ...
                    if (_availableDatesForReschedule.isNotEmpty)
                      ...
                      const Text(
                        "Tidak ada tanggal tersedia untuk penggantian jadwal.",
                      ),
                          ...
                          _isLoadingData
                              ...
                              : GridView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  bool isBooked = _bookedSlotsForNewDate
                                      .containsKey(hour);
                                  bool isSelected = _selectedNewHourIndexes
                                      .contains(hour);
                                  bool isPast =
                                      _selectedNewDate.isAtSameMomentAs(
                                        DateTime(
                                          DateTime.now().year,
                                          DateTime.now().month,
                                          DateTime.now().day,
                                  ...
                                  return ElevatedButton(
                                    onPressed:
                                        (isBooked || isPast || _isRescheduling)
                                            ? null
                                            : () {
                                              setState(() {
                                                if (isSelected) {
                                                  _selectedNewHourIndexes
                                                      .remove(hour);
                                                } else {
                                                  if (_selectedNewHourIndexes
                                                          .length <
                                                      _originalDurationInHours) {
                                                    _selectedNewHourIndexes.add(
                                                      hour,
                                                    );
                                                  } else {
                                                    ScaffoldMessenger.of(
                                                      ...
                                                      SnackBar(
                                                        content: Text(
                                                          "Anda hanya bisa memilih $_originalDurationInHours jam.",
                                                        ),
                                                      ),
                                                    );
                                                  }
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed:
                            (_isRescheduling ||
                                    !_isSelectionValid(_selectedNewHourIndexes))
                                ? null
                                : _confirmReschedule,
                        style: ElevatedButton.styleFrom(
                          backgroundColor: Colors.blue,
                          // Menonaktifkan tombol jika pilihan tidak valid
